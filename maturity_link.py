...
- Пиши только на русском."""
    ...
    try:
        response = _chat_completions_with_model_fallback(
            ...
            rec_models,
            messages=[
                {"role": "system", "content": "Ты Agile-коуч. Даёшь исполнимые шаги и явную выгоду для бизнеса. Структурированный план только на основе данных оценки. Язык ответа — русский."},
...
Дай практичные советы на **следующий квартал**: что сделать, чтобы команда **поняла**, есть ли это у них (эксперименты, вопросы на ретро, короткий опрос, парные интервью, наблюдение за потоком и т.д.).
...
"""
    try:
        response = _chat_completions_with_model_fallback(
            ...
            messages=[
                {"role": "system", "content": "Ты практик Agile-трансформаций в enterprise. Даёшь планы стрима с исполнимыми шагами и явной выгодой для бизнеса. Язык — русский."},
                {"role": "user", "content": prompt},
            ],
            temperature=0.45,
            max_tokens=3500,
