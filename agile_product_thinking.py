        ...
        "cases": [
            {
                ...
                "ready_stories": [
                    {
                        "story": "Как премиум-клиент, я хочу увидеть баннер новой премиум-карты на главном экране, чтобы быстро понять выгоду и перейти к оформлению.",
                        "details": "Добавляем entry-point в домашний экран мобильного банка для сегмента premium. Показываем оффер с 3 ключевыми выгодами и CTA в заявку.",
                        "acceptance": [
                            "Баннер виден только пользователям из premium-сегмента по CRM-флагу",
                            "Нажатие на CTA открывает экран заявки, время открытия < 2 секунд",
                            "Показы и клики пишутся в аналитику как отдельные события",
                        ],
                    },
                    {
                        "story": "Как действующий клиент банка, я хочу подать заявку на премиум-карту в приложении за 2–3 шага, чтобы не идти в офис.",
                        "details": "Форма заявки с автоподтягиванием профиля клиента, чекбоксом оферты и подтверждением по SMS/биометрии.",
                        "acceptance": [
                            "Поля ФИО и телефон предзаполнены из профиля и доступны для проверки",
                            "Без принятия условий кнопка отправки заблокирована",
                            "После успешной отправки создаётся заявка в процессинге со статусом created",
                        ],
                    },
                    {
                        "story": "Как клиент, я хочу выбрать способ доставки карты и адрес прямо в приложении, чтобы получить карту удобным способом.",
                        "details": "Экран выбора: курьер / отделение + таймслот. Поддержка валидации адреса и отображение доступных окон доставки.",
                        "acceptance": [
                            "Доступны не менее двух способов получения: курьер и офис",
                            "Для курьера проверяется валидность адреса и зона покрытия",
                            "Выбранные параметры доставки сохраняются в заявке и видны в трекинге",
                        ],
                    },
                    {
                        "story": "Как клиент, я хочу активировать полученную карту в приложении, чтобы начать ею пользоваться без звонка в поддержку.",
                        "details": "Активация по последним 4 цифрам карты, подтверждение через биометрию/PIN и установка PIN-кода для операций.",
                        "acceptance": [
                            "Активация возможна только для статуса карты delivered",
                            "После активации статус карты меняется на active и приходит push",
                            "Ошибка активации показывает понятную причину и следующий шаг",
                        ],
                    },
                    {
                        "story": "Как клиент, я хочу видеть в истории операций категорию и ставку кешбэка по каждой покупке, чтобы понимать начисления.",
                        "details": "Расширяем строку транзакции: MCC-категория, применённая ставка, ожидаемый бонус. Данные приходят из loyalty-сервиса.",
                        "acceptance": [
                            "Для транзакции отображается категория и ставка кешбэка",
                            "При отсутствии категории показывается fallback «Другое» со ставкой по умолчанию",
                            "Сумма ожидаемого кешбэка совпадает с расчётом backend",
                        ],
                    },
                    {
                        "story": "Как клиент, я хочу видеть дашборд «Кешбэк за месяц», чтобы быстро оценить текущую выгоду по карте.",
                        "details": "Новый виджет на карточке продукта: сумма за месяц, топ-3 категории, прогноз до конца месяца.",
                        "acceptance": [
                            "Виджет показывает текущую сумму кешбэка за календарный месяц",
                            "Топ-3 категорий сортируется по сумме начислений",
                            "При пустых данных отображается корректный пустой state без ошибок",
                        ],
                    },
                    {
                        "story": "Как клиент, я хочу выбирать приоритетные категории кешбэка на месяц, чтобы получать больше выгоды в своих тратах.",
                        "details": "Экран выбора категорий с ограничением по количеству и датой фиксации выбора до конца периода.",
                        "acceptance": [
                            "Клиент может выбрать не более 3 категорий из доступного списка",
                            "Изменение категории после дедлайна блокируется и объясняется в UI",
                            "Выбранные категории используются в расчёте ставок с ближайшего расчётного периода",
                        ],
                    },
                    {
                        "story": "Как клиент, я хочу тратить накопленный кешбэк на компенсацию покупок, чтобы сразу видеть финансовую пользу.",
                        "details": "Сценарий redemption: выбор операции из списка подходящих, подтверждение списания бонусов, отображение результата.",
                        "acceptance": [
                            "Компенсация доступна только для операций, удовлетворяющих правилам программы",
                            "После подтверждения баланс бонусов уменьшается атомарно",
                            "История операции получает отметку о компенсации и сумму списания",
                        ],
                    },
                    {
                        "story": "Как держатель премиум-карты, я хочу выпустить виртуальную карту для члена семьи, чтобы разделять траты и сохранять контроль.",
                        "details": "Флоу создания дополнительной виртуальной карты: имя держателя, лимиты, мгновенный выпуск и привязка к общему счёту.",
                        "acceptance": [
                            "Допкарта выпускается без физической доставки за время < 1 минуты",
                            "Для допкарты можно задать дневной и месячный лимит",
                            "Операции по допкарте видны в общей истории с признаком владельца",
                        ],
                    },
                    {
                        "story": "Как клиент, я хочу получать пуш о начисленном кешбэке после покупки, чтобы сразу понимать выгоду от программы.",
                        "details": "Триггерим уведомление после расчёта начисления: сумма покупки, категория, размер кешбэка, ссылка в детали.",
                        "acceptance": [
                            "Push отправляется только после успешного расчёта начисления",
                            "Текст push содержит сумму кешбэка и категорию покупки",
                            "Тап по уведомлению открывает экран деталей транзакции",
                        ],
                    },
                    {
                        "story": "Как премиум-клиент, я хочу открыть чат с консьерж-сервисом из карточки продукта, чтобы быстро решать travel/lifestyle-запросы.",
                        "details": "Интеграция с внешним concierge-провайдером: старт чата, передача идентификатора клиента, SLA-метка в интерфейсе.",
                        "acceptance": [
                            "Кнопка «Консьерж» доступна только активным премиум-картам",
                            "Чат открывается сессией текущего пользователя без повторного логина",
                            "В интерфейсе отображается ожидаемое время ответа оператора",
                        ],
                    },
                    {
                        "story": "Как сотрудник поддержки, я хочу видеть полный статус пути клиента по премиум-карте, чтобы быстро помогать без эскалаций.",
                        "details": "Back-office экран со статусами: заявка, доставка, активация, начисления, последние ошибки интеграций.",
                        "acceptance": [
                            "Карточка поддержки показывает актуальные статусы всех ключевых этапов",
                            "Последняя техническая ошибка видна с timestamp и кодом причины",
                            "Оператор может скопировать тех.сводку одним действием для эскалации",
                        ],
                    },
                ],
            ...
            },
            {
                ...
                "ready_stories": [
                    {
                        "story": "Как участник выезда, я хочу получить приглашение с программой и дедлайнами, чтобы заранее понимать формат и подготовиться.",
                        "details": "Рассылка приглашения по корпоративной почте и в Slack с описанием целей, дат, FAQ и ссылкой на регистрацию.",
                        "acceptance": [
                            "Приглашение отправлено 100% сотрудникам из списка участия",
                            "В письме есть дата, место, программа и ссылка на форму регистрации",
                            "Открытия и переходы по ссылке фиксируются в отчёте оргкомитета",
                        ],
                    },
                    {
                        "story": "Как организатор, я хочу собрать регистрацию участников с ограничениями по питанию и проживанию, чтобы корректно спланировать логистику.",
                        "details": "Форма регистрации в HRM с обязательными полями: участие, диета, аллергии, город вылета, пожелания по размещению.",
                        "acceptance": [
                            "Без заполнения обязательных полей форма не отправляется",
                            "Ответы участников доступны оргкомитету в единой таблице",
                            "Изменение ответа участником возможно до установленного дедлайна",
                        ],
                    },
                    {
                        "story": "Как удалённый сотрудник, я хочу видеть варианты трансфера и точки сбора, чтобы без стресса добраться до площадки.",
                        "details": "Публикуем персонализированные инструкции по логистике: автобусы, авиа/жд для иногородних, контакт координатора.",
                        "acceptance": [
                            "Каждый участник получает маршрут, соответствующий его городу",
                            "Указаны время, точка сбора и контакты ответственного",
                            "Любое изменение рейса/автобуса обновляется и дублируется уведомлением",
                        ],
                    },
                    {
                        "story": "Как офис-менеджер, я хочу зафиксировать договор с площадкой и кейтерингом, чтобы минимизировать риск срыва мероприятия.",
                        "details": "Заключаем и проверяем договоры: вместимость, меню, тайминг питания, штрафы, план Б при плохой погоде.",
                        "acceptance": [
                            "Подписаны договоры с площадкой и кейтерингом с согласованной сметой",
                            "В договоре есть условия отмены и штрафные санкции",
                            "Согласован резервный сценарий проведения в помещении",
                        ],
                    },
                    {
                        "story": "Как участник, я хочу заранее увидеть распределение по номерам, чтобы понимать условия проживания и соседей.",
                        "details": "Формируем рассадку по номерам на основе пола, предпочтений и ограничений; публикуем за 3 дня до выезда.",
                        "acceptance": [
                            "Список размещения доступен всем участникам не позднее чем за 72 часа",
                            "Учтены обязательные ограничения (пол, медицинские особенности)",
                            "Есть процесс запроса на изменение размещения",
                        ],
                    },
                    {
                        "story": "Как фасилитатор, я хочу иметь чёткий тайминг стратегической сессии, чтобы уложиться в 3 часа и получить результат.",
                        "details": "Подготавливаем структуру сессии: цели, блоки по 30–40 минут, артефакты и ответственные за фиксацию решений.",
                        "acceptance": [
                            "Повестка разбита на блоки с временем начала и конца",
                            "Для каждого блока назначен ведущий и формат фиксации результатов",
                            "По итогам сессии сформирован единый протокол решений",
                        ],
                    },
                    {
                        "story": "Как участник, я хочу видеть финальную программу двух дней в одном месте, чтобы спланировать своё участие.",
                        "details": "Публикуем программу в карточке события: активности, сессии, окна отдыха, время питания и трансферов.",
                        "acceptance": [
                            "Программа содержит все активности с временем и локацией",
                            "Изменения в программе обновляются в течение 30 минут после решения",
                            "Участники могут скачать/сохранить программу офлайн",
                        ],
                    },
                    {
                        "story": "Как организатор, я хочу иметь список рисков и план реагирования, чтобы не терять контроль в день мероприятия.",
                        "details": "Регистр рисков: опоздание транспорта, плохая погода, срыв питания, мед.инциденты, отсутствие ведущего.",
                        "acceptance": [
                            "Для каждого критичного риска определён owner и шаги реагирования",
                            "Контакты экстренных служб и подрядчиков собраны в один документ",
                            "Команда организаторов проходит короткий брифинг по рискам до выезда",
                        ],
                    },
                    {
                        "story": "Как участник с ограничениями по питанию, я хочу получить подходящее меню, чтобы комфортно участвовать в мероприятии.",
                        "details": "Сегментация меню по диетам и аллергиям, маркировка блюд на площадке и отдельная линия выдачи при необходимости.",
                        "acceptance": [
                            "Все заявленные диетические ограничения покрыты в меню",
                            "Блюда маркированы понятными тегами (vegan, gluten-free и т.д.)",
                            "На площадке назначен ответственный за вопросы по питанию",
                        ],
                    },
                    {
                        "story": "Как организатор, я хочу организовать чек-ин на площадке, чтобы быстро понимать, кто уже прибыл.",
                        "details": "Реестр прибытия по автобусам и самостоятельному трансферу, отметка присутствия и канал для опоздавших.",
                        "acceptance": [
                            "Статус прибытия фиксируется по каждому участнику в день события",
                            "Список опаздывающих обновляется в реальном времени для координатора",
                            "После чек-ина участник получает информацию о первом активити",
                        ],
                    },
                    {
                        "story": "Как HR-директор, я хочу собрать обратную связь в течение суток после выезда, чтобы оценить эффект и улучшить следующий ретрит.",
                        "details": "Опрос NPS + качественные вопросы: полезность сессий, логистика, питание, предложения по улучшению.",
                        "acceptance": [
                            "Опрос отправлен всем участникам не позднее 24 часов после завершения",
                            "Доля ответов не ниже 70% за 3 дня",
                            "Подготовлен краткий отчёт с топ-3 улучшениями для следующего события",
                        ],
                    },
                    {
                        "story": "Как оргкомитет, я хочу закрыть финансовый отчёт по выезду, чтобы подтвердить бюджет и прозрачность расходов.",
                        "details": "Сверка актов, счетов и фактических трат по категориям: площадка, транспорт, питание, активити, мерч.",
                        "acceptance": [
                            "Фактические расходы сведены в отчёт по согласованным бюджетным статьям",
                            "Отклонения от бюджета >10% имеют текстовое обоснование",
                            "Финальный отчёт согласован CFO и HR-директором",
                        ],
                    },
                ],
