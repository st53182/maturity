<template>
  <div class="wsjf-fac">
    <header class="wsjf-fac__head">
      <div>
        <h1>⚖️ {{ $t('agileTraining.wsjf.facTitle') }}</h1>
        <p class="wsjf-fac__sub">{{ $t('agileTraining.wsjf.facSubtitle') }}</p>
      </div>
      <div>
        <router-link class="wsjf-fac__back" to="/agile-training">
    ...
    <!-- список сессий -->
    <section v-if="!activeSession" class="wsjf-fac__section">
      <h2>{{ $t('agileTraining.facilitator.mySessions') }}</h2>
      <form class="wsjf-fac__create" @submit.prevent="createSession">
        <input v-model="newSessionTitle"
               :placeholder="$t('agileTraining.wsjf.newSessionTitle')"
               required maxlength="255" />
        <select v-model="newSessionLocale" class="wsjf-fac__locale">
      ...
      <div v-if="loadingSessions" class="wsjf-fac__hint">{{ $t('common.loading') }}…</div>
      <div v-else-if="sessions.length === 0" class="wsjf-fac__empty">
        {{ $t('agileTraining.facilitator.noSessions') }}
      </div>
      <ul v-else class="wsjf-fac__list">
        <li v-for="s in sessions" :key="s.id" class="wsjf-fac__item">
          <div class="wsjf-fac__item-main">
            <div class="wsjf-fac__item-title">{{ s.title }}</div>
            <div class="wsjf-fac__item-meta">
              <span class="wsjf-fac__badge">{{ (s.locale || 'ru').toUpperCase() }}</span>
          ...
          <button class="wsjf-fac__open-btn" @click="openSession(s.id)">
    ...
    <!-- активная сессия -->
    <section v-else class="wsjf-fac__section">
      <div class="wsjf-fac__active-head">
        <div>
          <div class="wsjf-fac__active-title">{{ activeSession.title }}</div>
          <div class="wsjf-fac__item-meta">
            <span class="wsjf-fac__badge">{{ (activeSession.locale || 'ru').toUpperCase() }}</span>
            <span>{{ formatDate(activeSession.created_at) }}</span>
          </div>
        </div>
        <div class="wsjf-fac__active-actions">
          <button class="btn-ghost" @click="openCompareAll" :disabled="!groups.length">
            🏁 {{ $t('agileTraining.wsjf.compareAll') }}
          </button>
      ...
      <div class="wsjf-fac__groups-head">
        <h3>{{ $t('agileTraining.facilitator.groups') }}</h3>
        <form class="wsjf-fac__add-group" @submit.prevent="addGroup">
      ...
      <div v-if="groups.length === 0" class="wsjf-fac__empty">
        {{ $t('agileTraining.facilitator.noGroups') }}
      </div>
      <ul v-else class="wsjf-fac__groups">
        <li v-for="g in groups" :key="g.id" class="wsjf-fac__group">
          <div class="wsjf-fac__group-main">
            <div class="wsjf-fac__group-name">{{ g.name }}</div>
            <div class="wsjf-fac__item-meta">
              <span>{{ $t('agileTraining.facilitator.participants',
                           { n: g.participants_count || 0 }, g.participants_count || 0) }}</span>
              <span>·</span>
              <span>{{ $t('agileTraining.wsjf.answersCount',
                           { n: g.answers_count || 0 }, g.answers_count || 0) }}</span>
            </div>
            <div class="wsjf-fac__link">
              <code>{{ publicLink(g.slug) }}</code>
              <button class="wsjf-fac__copy" @click="copyLink(g.slug)">
                {{ copiedSlug === g.slug
          ...
          <div class="wsjf-fac__group-actions">
    ...
    <div v-if="resultsModal.open" class="wsjf-modal" @click.self="closeResults">
      <div class="wsjf-modal__body">
        <div class="wsjf-modal__head">
          <h3>{{ $t('agileTraining.wsjf.groupResults') }}: {{ resultsModal.group?.name }}</h3>
          <button class="wsjf-modal__close" @click="closeResults">✕</button>
        </div>

        <div v-if="resultsModal.loading" class="wsjf-fac__hint">{{ $t('common.loading') }}…</div>
        <div v-else-if="resultsModal.data">
          <p class="wsjf-modal__lead">
          ...
          <!-- adaptation pills -->
          <div class="wsjf-adapt">
            <span v-for="(cnt, key) in resultsModal.data.adaptation_counts"
                  :key="key"
                  class="wsjf-pill"
                  :class="'wsjf-pill--' + key">
              {{ $t('agileTraining.wsjf.adaptation.' + key) }}: <b>{{ cnt }}</b>
            </span>
          ...
          <h4 class="wsjf-section-title">🛣️ {{ $t('agileTraining.wsjf.choicesFlow') }}</h4>
          ...
          <div class="wsjf-cols">
            <div>
              <h4>🎭 {{ $t('agileTraining.wsjf.byRole') }}</h4>
              <ul class="wsjf-roles">
                <li v-for="r in resultsModal.data.by_role" :key="r.role_key">
                  <b>{{ r.role_title }}</b>
                  <span class="wsjf-fac__hint">
                    · {{ $t('agileTraining.facilitator.participants',
                           { n: r.participants }, r.participants) }}
                  </span>
                  <ul class="wsjf-roles__picks">
                    <li v-for="(cnt, key) in r.revised" :key="'rev-' + key">
                      <b>{{ optionTitle(key) }}</b>: {{ cnt }}
                    </li>
                    <li v-if="!Object.keys(r.revised || {}).length" class="wsjf-fac__hint">
                      — {{ $t('agileTraining.wsjf.noRevisedYet') }}
                    </li>
                  </ul>
                </li>
              </ul>
            ...
            <div>
              <h4>⚠️ {{ $t('agileTraining.wsjf.topErrors') }}</h4>
              <ul class="wsjf-errors">
                <li v-for="(cnt, key) in resultsModal.data.error_counts" :key="key">
                  <span>{{ $t('agileTraining.wsjf.err.' + key) }}</span>
                  <b>{{ cnt }}</b>
                </li>
                <li v-if="!Object.keys(resultsModal.data.error_counts || {}).length"
                    class="wsjf-fac__hint">
                  — {{ $t('agileTraining.wsjf.noErrors') }}
                </li>
              </ul>

              <h4 style="margin-top: 14px">🗞 {{ $t('agileTraining.wsjf.eventsShown') }}</h4>
              <ul class="wsjf-errors">
          ...
          <!-- drill-down -->
          <div v-if="resultsModal.participants.length" class="wsjf-participants">
            <button class="wsjf-fac__participants-toggle"
                    @click="resultsModal.showParticipants = !resultsModal.showParticipants">
              {{ resultsModal.showParticipants ? '▾' : '▸' }}
              {{ $t('agileTraining.wsjf.participantsDrill') }}
              ({{ resultsModal.participants.length }})
            </button>
            <div v-if="resultsModal.showParticipants" class="wsjf-participants__body">
              <div v-for="p in resultsModal.participants" :key="p.id" class="wsjf-part">
                <button class="wsjf-part__toggle" @click="togglePart(p.id)">
                  <span>{{ resultsModal.expanded[p.id] ? '▾' : '▸' }}</span>
                  <b>{{ p.display_name }}</b>
                  <span v-if="p.has_answer" class="wsjf-pill wsjf-pill--muted">
                    {{ p.role_title || p.role_key }}
                  </span>
                  <span v-if="p.has_answer && p.initial_choice" class="wsjf-pill">
                  ...
                  <span v-if="p.has_answer && p.adaptation"
                        class="wsjf-pill"
                        :class="'wsjf-pill--' + p.adaptation">
                    {{ $t('agileTraining.wsjf.adaptation.' + p.adaptation) }}
                  </span>
                  <span v-if="!p.has_answer" class="wsjf-pill wsjf-pill--muted">
                    {{ $t('agileTraining.wsjf.noAnswer') }}
                  </span>
                </button>
                <div v-if="resultsModal.expanded[p.id] && p.has_answer" class="wsjf-part__body">
                  <div v-if="p.event" class="wsjf-part__event">
                    🗞 <b>{{ p.event.title }}</b>
                    <div class="wsjf-fac__hint">{{ p.event.lead }}</div>
                  </div>
                  <template v-for="stage in ['initial', 'revised']" :key="stage">
                    <div v-if="p[stage]" class="wsjf-part__stage">
                      <div class="wsjf-part__stage-title">
                        {{ $t('agileTraining.wsjf.round.' + stage) }}
                        <span v-if="p[stage].choice" class="wsjf-pill">
                          {{ p[stage].choice.title }}
                        </span>
                      </div>
                      <table class="wsjf-table wsjf-table--compact">
                        <thead>
                          <tr>
                            <th>{{ $t('agileTraining.wsjf.option') }}</th>
                      ...
                      <div v-if="p[stage].eval && p[stage].eval.errors && p[stage].eval.errors.length"
                           class="wsjf-part__errors">
                        <span v-for="er in p[stage].eval.errors" :key="er" class="wsjf-pill wsjf-pill--warn">
    ...
    <div v-if="compareAll.open" class="wsjf-modal" @click.self="closeCompareAll">
      <div class="wsjf-modal__body wsjf-modal__body--wide">
        <div class="wsjf-modal__head">
          <h3>🏁 {{ $t('agileTraining.wsjf.compareAll') }}</h3>
          <button class="wsjf-modal__close" @click="closeCompareAll">✕</button>
        </div>
        <div v-if="compareAll.loading" class="wsjf-fac__hint">{{ $t('common.loading') }}…</div>
        <div v-else-if="compareAll.data">
          <h4 class="wsjf-section-title">📊 {{ $t('agileTraining.wsjf.groupScoreboard') }}</h4>
          <table class="wsjf-table">
            <thead>
              <tr>
                <th>{{ $t('agileTraining.facilitator.groups') }}</th>
                <th>{{ $t('agileTraining.facilitator.participants', { n: 2 }, 2) }}</th>
                <th>{{ $t('agileTraining.wsjf.initialChoice') }}</th>
                <th>{{ $t('agileTraining.wsjf.revisedChoice') }}</th>
                <th>{{ $t('agileTraining.wsjf.changedMind') }}</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="g in compareAll.data.groups" :key="g.id">
                ...
                <td>
                  <span v-for="(cnt, key) in g.initial_counts" :key="'i-' + key"
                        class="wsjf-pill wsjf-pill--muted">
                    {{ optionTitleFromCompare(key) }}: {{ cnt }}
                  </span>
                  <span v-if="!Object.keys(g.initial_counts || {}).length" class="wsjf-fac__hint">—</span>
                </td>
                <td>
                  ...
                  <span v-if="!Object.keys(g.revised_counts || {}).length" class="wsjf-fac__hint">—</span>
                </td>
...
export default {
  ...
  methods: {
    ...
    async createSession() {
      ...
      try {
        const r = await axios.post('/api/agile-training/sessions',
          { title, locale: this.newSessionLocale, exercise_key: 'wsjf' },
          { headers: this.authHeaders() });
    ...
    async resetGroup(g) {
      ...
      try {
        await axios.post(`/api/agile-training/wsjf/groups/${g.id}/reset`, {},
          { headers: this.authHeaders() });
        await this.openSession(this.activeSession.id);
      } catch (e) { alert(e.response?.data?.error || 'Failed'); }
    },
    ...
    async openGroupResults(g) {
      ...
      try {
        const r = await axios.get(`/api/agile-training/wsjf/groups/${g.id}/results`, {
          headers: this.authHeaders(),
      ...
      try {
        const parts = await axios.get(
          `/api/agile-training/wsjf/groups/${g.id}/participants`,
          { headers: this.authHeaders(),
            params: { locale: this.activeSession?.locale || 'ru' } },
        );
    ...
    async openCompareAll() {
      ...
      try {
        const r = await axios.get(
          `/api/agile-training/wsjf/sessions/${this.activeSession.id}/results`,
          { headers: this.authHeaders(),
            params: { locale: this.activeSession?.locale || 'ru' } },
        );
...
.wsjf-fac__head h1 { margin: 0 0 6px; font-size: 26px; }
.wsjf-fac__sub { margin: 0; color: #5a5a80; font-size: 14px; }
.wsjf-fac__back {
  color: #4f46e5;
...
.wsjf-fac__back:hover { background: #f3f3ff; }
...
.wsjf-fac__section h2 { margin: 0 0 12px; font-size: 18px; }
.wsjf-fac__section h3 { margin: 0; font-size: 16px; }

.wsjf-fac__create, .wsjf-fac__add-group {
  display: flex;
  gap: 8px;
  margin-bottom: 14px;
  flex-wrap: wrap;
}
.wsjf-fac__create input,
.wsjf-fac__add-group input,
.wsjf-fac__locale {
  flex: 1 1 220px;
  padding: 10px 12px;
...
.wsjf-fac__create button,
.wsjf-fac__add-group button {
...
.wsjf-fac__create button:hover,
.wsjf-fac__add-group button:hover {
  background: #4f46e5 !important;
  border-color: #4f46e5 !important;
  box-shadow: 0 4px 10px rgba(79, 70, 229, .25);
}
.wsjf-fac__create button:active,
.wsjf-fac__add-group button:active {
  transform: translateY(1px);
  background: #4338ca !important;
  border-color: #4338ca !important;
}

.wsjf-fac__list, .wsjf-fac__groups {
  list-style: none;
  margin: 0;
  padding: 0;
  display: flex;
...
.wsjf-fac__item, .wsjf-fac__group {
  display: flex;
  justify-content: space-between;
  align-items: center;
  gap: 12px;
...
.wsjf-fac__item-title, .wsjf-fac__group-name { font-weight: 600; font-size: 15px; }
.wsjf-fac__item-meta {
  display: flex;
  align-items: center;
  gap: 8px;
...
.wsjf-fac__copy, .wsjf-fac__open-btn {
  background: #6366f1 !important;
  border: 1px solid #6366f1 !important;
  color: #fff;
  border-radius: 8px;
...
.wsjf-fac__copy:hover, .wsjf-fac__open-btn:hover {
  background: #4f46e5 !important;
  border-color: #4f46e5 !important;
  box-shadow: 0 4px 10px rgba(79, 70, 229, .25);
}
.wsjf-fac__copy:active, .wsjf-fac__open-btn:active {
  transform: translateY(1px);
  background: #4338ca !important;
  border-color: #4338ca !important;
}

.wsjf-fac__active-head {
  display: flex;
  justify-content: space-between;
  align-items: flex-start;
  gap: 12px;
...
.wsjf-fac__active-title { font-size: 20px; font-weight: 700; }
.wsjf-fac__active-actions {
  display: flex;
  gap: 8px;
  flex-wrap: wrap;
}
.wsjf-fac__groups-head {
  display: flex;
  justify-content: space-between;
  gap: 10px;
  margin: 18px 0 10px;
...
.wsjf-modal__body--wide { max-width: 1200px; }
.wsjf-modal__head {
  display: flex;
  justify-content: space-between;
  align-items: center;
...
.wsjf-modal__head h3 { margin: 0; font-size: 18px; }
.wsjf-modal__close {
...
.wsjf-pill--muted { background: #f0f0f6; color: #6b6b8c; }
.wsjf-pill--warn { background: #fff3e0; color: #a85a00; }
.wsjf-pill--stayed_right { background: #e9f7ec; color: #1f7a3b; }
.wsjf-pill--adapted_well { background: #e5f1ff; color: #2057b8; }
.wsjf-pill--stayed_risky { background: #fff0f0; color: #a83030; }
.wsjf-pill--over_adjusted { background: #fff3e0; color: #a85a00; }
.wsjf-pill--no_change { background: #f0f0f6; color: #6b6b8c; }
...
.wsjf-table--compact { font-size: 12.5px; margin-bottom: 4px; }
.wsjf-row--pick {
  background: #eef0ff;
}
.wsjf-bar {
...
@media (max-width: 800px) {
  .wsjf-cols { grid-template-columns: 1fr; }
}
.wsjf-roles, .wsjf-errors {
  list-style: none;
  margin: 0;
  padding: 0;
  display: flex;
...
.wsjf-fac__participants-toggle:hover {
  background: #f3f3ff !important;
}
.wsjf-participants__body {
  margin-top: 10px;
  display: flex;
  flex-direction: column;
  gap: 8px;
...
.wsjf-part__toggle:hover { color: #4f46e5; }
.wsjf-part__body {
  padding: 10px 0 4px;
  border-top: 1px dashed #eee6ff;
  margin-top: 6px;
...
.wsjf-part__stage { margin-bottom: 10px; }
.wsjf-part__stage-title {
  display: flex;
  align-items: center;
  gap: 8px;
  font-weight: 600;
